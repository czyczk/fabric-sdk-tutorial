...
func TestKeySwitchProcess(t *testing.T) {
	...
	// 验证 ZKP
	// 验证份额 1 的 ZKP
	isShare1Verified, err := ppks.ShareProofVryNoB(proof1.C, proof1.R1, proof1.R2, share1, &participant1KeyInSM2.PublicKey, targetPubKeyInSM2, &encryptedKey.K)
	if isNoError := assert.NoError(t, err); !isNoError {
		t.FailNow()
	}
	if !isShare1Verified {
	...
	// 验证份额 2 的 ZKP
	isShare2Verified, err := ppks.ShareProofVryNoB(proof2.C, proof2.R1, proof2.R2, share2, &participant2KeyInSM2.PublicKey, targetPubKeyInSM2, &encryptedKey.K)
	if isNoError := assert.NoError(t, err); !isNoError {
		t.FailNow()
	}
	if !isShare2Verified {
