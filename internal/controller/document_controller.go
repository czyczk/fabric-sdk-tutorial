...
func (c *DocumentController) handleCreateDocument(ctx *gin.Context) {
	...
	switch resourceType {
	case data.Plain:
		txCreationInfo, err = c.DocumentSvc.CreateDocument(document)
	case data.Encrypted:
		txCreationInfo, err = c.DocumentSvc.CreateEncryptedDocument(document, key, policy)
	case data.Offchain:
		txCreationInfo, err = c.DocumentSvc.CreateOffchainDocument(document, key, policy)
	}
